...
- (DDOSLogger *)osLogger {
    if (!_osLogger) {
        ...
        //自定义输出格式
        _osLogger.logFormatter = [[FQLogFormatter alloc] init];
    }
    return _osLogger;
}
...
- (DDFileLogger *)fileLogger {
    if (!_fileLogger) {
        ...
        FQLogFileManager *fileManager = [[FQLogFileManager alloc] init];
        _fileLogger = [[DDFileLogger alloc] initWithLogFileManager:fileManager];
        //使用自定义的Logformatter
        _fileLogger.logFormatter = [[FQFileLogFormatter alloc] init];
